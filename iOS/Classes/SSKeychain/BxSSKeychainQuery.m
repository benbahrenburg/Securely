...
#import "BxSSKeychainQuery.h"
#import "BxSSKeychain.h"

@implementation BxSSKeychainQuery
...
#ifdef BxSSKEYCHAIN_SYNCHRONIZATION_AVAILABLE
...
- (BOOL)save:(NSError *__autoreleasing *)error {
	OSStatus status = BxSSKeychainErrorBadArguments;
    ...
    if(status == errSecSuccess){
        ...
        CFTypeRef accessibilityType = [BxSSKeychain accessibilityType];
...
- (NSMutableDictionary *)query {
	...
	if ([[self class] isSynchronizationAvailable]) {
		...
		switch (self.synchronizationMode) {
			case BxSSKeychainQuerySynchronizationModeNo: {
			  value = @NO;
			  break;
			}
			case BxSSKeychainQuerySynchronizationModeYes: {
			  value = @YES;
			  break;
			}
			case BxSSKeychainQuerySynchronizationModeAny: {
...
+ (NSError *)errorWithCode:(OSStatus) code {
	...
	switch (code) {
		...
		case BxSSKeychainErrorBadArguments: message = NSLocalizedStringFromTable(@"SSKeychainErrorBadArguments", @"SSKeychain", nil); break;
	...
	return [NSError errorWithDomain:kBxSSKeychainErrorDomain code:code userInfo:userInfo];
}
